...
const ManageLivestock = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      setErrorMsg('');
      const res = await fetch('http://localhost:5000/api/livestock?limit=1000');
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/livestock/${editingItem._id}`, {
        method: 'PUT',
        body: formData
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/livestock/${id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
