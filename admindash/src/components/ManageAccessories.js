...
const ManageAccessories = () => {
  ...
  const fetchAllAccessories = async () => {
    try {
      setIsLoading(true);
      setErrorMsg('');
      const response = await fetch('http://https://aqualeads-project.onrender.com/api/accessories');
  ...
  const getImageUrl = (image) => {
    ...
    if (image.filename) return `http://https://aqualeads-project.onrender.com/uploads/accessories/${image.filename}`;
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://https://aqualeads-project.onrender.com/api/accessories/${editingAccessory._id}`, {
        method: 'PUT',
        body: formData
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://https://aqualeads-project.onrender.com/api/accessories/${id}`, {
        method: 'DELETE'
      });
      const data = await res.json();
