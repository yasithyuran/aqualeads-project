...
const ManageAriums = () => {
  ...
  const fetchAllAriums = async () => {
    try {
      setIsLoading(true);
      setErrorMsg('');
      const response = await fetch('http://https://aqualeads-project.onrender.com/api/ariums');
  ...
  const getImageUrl = (image) => {
    ...
    if (image.filename) return `http://https://aqualeads-project.onrender.com/uploads/ariums/${image.filename}`;
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://https://aqualeads-project.onrender.com/api/ariums/${editingArium._id}`, {
        method: 'PUT',
        body: formData
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://https://aqualeads-project.onrender.com/api/ariums/${id}`, {
        method: 'DELETE'
      });
      const data = await res.json();
