...
const ManageArticles = () => {
  ...
  const fetchAllArticles = async () => {
    try {
      setIsLoading(true);
      setErrorMsg('');
      const response = await fetch('http://https://aqualeads-project.onrender.com/api/articles');
  ...
  const getImageUrl = (article) => {
    if (article.frontPic && article.frontPic.filename) {
      return `http://https://aqualeads-project.onrender.com/uploads/${article.frontPic.filename}`;
    }
    if (article.frontPic && article.frontPic.path) {
      return `http://https://aqualeads-project.onrender.com${article.frontPic.path}`;
    }
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://https://aqualeads-project.onrender.com/api/articles/${editingArticle._id}`, {
        method: 'PUT',
        body: formData
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://https://aqualeads-project.onrender.com/api/articles/${id}`, {
        method: 'DELETE'
      });
      const data = await res.json();
