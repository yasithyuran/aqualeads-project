...
const ManageImEx = () => {
  ...
  const fetchAllPosts = async () => {
    try {
      setIsLoading(true);
      setErrorMsg('');
      const response = await fetch('http://localhost:5000/api/imex/all');
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/imex/${editingPost._id}`, {
        method: 'PUT',
        body: formData
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/imex/${id}`, {
        method: 'DELETE'
      });
      const data = await res.json();
