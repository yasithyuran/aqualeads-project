...
const ManageInterior = () => {
  ...
  const fetchInteriors = async () => {
    try {
      setLoading(true);
      setErrorMsg('');
      const res = await fetch('http://localhost:5000/api/interiors/all');
  ...
  const handleEditSubmit = async e => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/interiors/${editingInterior._id}`, {
        method: 'PUT',
        body: formData
      });
  ...
  const handleDelete = async id => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/interiors/${id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
