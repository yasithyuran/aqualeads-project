...
export default function Lowt() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://https://aqualead-project.onrender.com/api/ariums?subSubSubCategory=lowt');
      ...
      console.error('Error fetching low tech aquariums:', err);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  if (loading) return <div className="center">Loading Low Tech Aquariums...</div>;

  return (
    <div className="live-page">
      <div className="back-arrow" onClick={() => window.history.back()}>←</div>
      <div className="toolbar"><h2 style={{ fontWeight: 'bold', fontSize: '2rem', textAlign: 'left', marginBottom: '70px', marginTop: '30px', paddingLeft: '100px' }}>Low Tech Aquariums</h2></div>
      ...
      {!loading && items.length === 0 && <div className="center">No low tech aquariums available yet.</div>}
