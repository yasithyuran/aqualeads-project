...
export default function Scenarium() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch('https://aqualeads-project.onrender.com/api/ariums?mainCategory=scenarium');
      ...
      console.error('Error fetching scenariums:', err);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  if (loading) return <div className="center">Loading Scenariums...</div>;

  return (
    <div className="live-page">
      <div className="back-arrow" onClick={() => window.history.back()}>←</div>
      <div className="toolbar"> <h2 style={{ fontWeight: 'bold', fontSize: '2rem', textAlign: 'left', marginBottom: '70px', marginTop: '30px', paddingLeft: '100px' }}>Scenariums</h2></div>
      ...
      {!loading && items.length === 0 && <div className="center">No scenariums available yet.</div>}
