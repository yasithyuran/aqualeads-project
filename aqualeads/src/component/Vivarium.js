...
export default function Vivarium() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://https://aqualead-project.onrender.com/api/ariums?mainCategory=vivarium');
      ...
      console.error('Error fetching vivariums:', err);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  if (loading) return <div className="center">Loading Vivariums...</div>;

  return (
    <div className="live-page">
      <div className="back-arrow" onClick={() => window.history.back()}>←</div>
      <div className="toolbar"> <h2 style={{ fontWeight: 'bold', fontSize: '2rem', textAlign: 'left', marginBottom: '70px', marginTop: '30px', paddingLeft: '100px' }}>Vivariums</h2></div>
      ...
      {!loading && items.length === 0 && <div className="center">No vivariums available yet.</div>}
