...
export default function Pond() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://https://aqualead-project.onrender.com/api/ariums?mainCategory=pond');
  ...
  if (loading) return <div className="center">Loading Ponds...</div>;

  return (
    <div className="live-page">
      <div className="back-arrow" onClick={() => window.history.back()}>←</div>
      <div className="toolbar"> <h2 style={{ fontWeight: 'bold', fontSize: '2rem', textAlign: 'left', marginBottom: '70px', marginTop: '30px', paddingLeft: '100px' }}>Ponds</h2></div>
      ...
      {!loading && items.length === 0 && <div className="center">No ponds available yet.</div>}
