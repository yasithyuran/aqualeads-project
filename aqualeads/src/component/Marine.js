...
export default function Marine() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://https://aqualead-project.onrender.com/api/ariums?subCategory=marine');
      ...
      console.error('Error fetching marine aquariums:', err);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  if (loading) return <div className="center">Loading Marine Aquariums...</div>;

  return (
    <div className="live-page">
      <div className="back-arrow" onClick={() => window.history.back()}>←</div>
      <div className="toolbar"> <h2 style={{ fontWeight: 'bold', fontSize: '2rem', textAlign: 'left', marginBottom: '70px', marginTop: '30px', paddingLeft: '100px' }}>Marine Aquariums</h2></div>
      ...
      {!loading && items.length === 0 && <div className="center">No marine aquariums available yet.</div>}
