...
export default function Zero() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://https://aqualead-project.onrender.com/api/ariums?subSubSubCategory=zero');
      ...
      console.error('Error fetching zero aquariums:', err);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  if (loading) return <div className="center">Loading Zero Aquariums...</div>;

  return (
    <div className="live-page">
      <div className="back-arrow" onClick={() => window.history.back()}>←</div>
      <div className="toolbar"> <h2 style={{ fontWeight: 'bold', fontSize: '2rem', textAlign: 'left', marginBottom: '70px', marginTop: '30px', paddingLeft: '100px' }}>Zero Maintain Aquariums</h2></div>
      ...
      {!loading && items.length === 0 && <div className="center">No zero aquariums available yet.</div>}
